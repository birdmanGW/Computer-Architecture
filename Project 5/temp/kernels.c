 ...
 */
team_t team = {
    "Seans Optimized Code",              /* Team name */

    "Sean N. Bird",     /* First member full name */
    "snbbrbrd@gwmail.gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
	...
	for (j = 0; j < dim; j+=32) {
		...
		for (i = 0; i < dim; i++) {
			...
			/*
			 * As the code block continues
			 * we must also be incrementing how much we're
			 * mapping by j code chunking to make it easier to access
			 */
		...
		for (i = 0; i < dim; i++) {
			...
			for(ii = ((i-1 > 0) ? i-1 : 0); ii <= ((i+1 < dim-1) ? i+1 : dim-1); ii++) {
				for(jj = ((j-1 > 0) ? j-1 : 0); jj <= ((j+1 < dim-1) ? j+1 : dim-1); jj++) {
					...
					//set srcPtr
					//srcPtr = &src[RIDX(ii, jj, dim)];
					
					//Accumulates field values of src in corresponding fields of sum
					sum.red += (int) srcPtr->red;
					sum.green += (int) srcPtr->green;
					sum.blue += (int) srcPtr->blue;
					sum.num++;
				} 
				/*
				//Computed average pixel value in current_pixel
			current_pixel.red = (unsigned short) (sum.red/sum.num);
			current_pixel.green = (unsigned short) (sum.green/sum.num);
			current_pixel.blue = (unsigned short) (sum.blue/sum.num);
